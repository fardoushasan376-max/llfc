...
<style>
:root{
  --ucl-navy:#0A1628;--ucl-blue:#1B3A6B;--ucl-mid:#2A5298;--ucl-light:#4472C4;
  --ucl-bright:#5B8FE8;--ucl-star:#F5C518;--ucl-silver:#C8D8F0;--ucl-white:#EEF4FF;
  --ucl-dark:#060E1C;--ucl-card:#0D1F3C;--ucl-border:rgba(68,114,196,0.35);
...
body{font-family:'Montserrat',sans-serif;background:#060E1C;color:#EEF4FF;min-height:100vh;overflow-x:hidden;}
body::before{content:'';position:fixed;inset:0;background:radial-gradient(ellipse at 20% 20%,rgba(27,58,107,0.5) 0%,transparent 50%),radial-gradient(ellipse at 80% 80%,rgba(42,82,152,0.3) 0%,transparent 50%);pointer-events:none;z-index:0;}
body::after{content:'';position:fixed;inset:0;background-image:radial-gradient(1px 1px at 10% 15%,rgba(245,197,24,0.6) 0%,transparent 100%),radial-gradient(1px 1px at 30% 40%,rgba(245,197,24,0.4) 0%,transparent 100%),radial-gradient(1px 1px at 70% 60%,rgba(245,197,24,0.5) 0%,transparent 100%),radial-gradient(1px 1px at 85% 20%,rgba(255,255,255,0.25) 0%,transparent 100%),radial-gradient(1px 1px at 15% 70%,rgba(245,197,24,0.4) 0%,transparent 100%);pointer-events:none;z-index:0;}

header{position:sticky;top:0;z-index:1000;background:rgba(6,14,28,0.97);backdrop-filter:blur(16px);border-bottom:1px solid var(--ucl-border);box-shadow:0 4px 30px rgba(27,58,107,0.5);}
...
.logo-badge{width:44px;height:44px;background:linear-gradient(135deg,var(--ucl-mid),var(--ucl-blue));border:2px solid var(--ucl-light);border-radius:50%;display:flex;align-items:center;justify-content:center;font-family:'Bebas Neue',sans-serif;font-size:13px;color:white;box-shadow:0 0 16px rgba(68,114,196,0.5);flex-shrink:0;letter-spacing:0.5px;}
.logo-text{display:flex;flex-direction:column;line-height:1.1;}
.logo-main{font-family:'Bebas Neue',sans-serif;font-size:22px;letter-spacing:3px;color:#EEF4FF;}
...
.nav-btn{background:none;border:1px solid transparent;color:#C8D8F0;padding:7px 14px;font-family:'Montserrat',sans-serif;font-weight:600;font-size:12px;letter-spacing:1px;cursor:pointer;border-radius:4px;transition:all .2s;text-transform:uppercase;}
.nav-btn:hover,.nav-btn.active{color:#EEF4FF;border-color:var(--ucl-light);background:rgba(68,114,196,0.18);}
.header-auth{display:flex;gap:6px;align-items:center;flex-shrink:0;}
.btn-login{background:transparent;border:1px solid var(--ucl-light);color:var(--ucl-bright);padding:6px 14px;font-family:'Montserrat',sans-serif;font-weight:700;font-size:12px;cursor:pointer;border-radius:4px;transition:all .2s;text-transform:uppercase;}
.btn-login:hover{background:var(--ucl-mid);color:white;}
.btn-register{background:linear-gradient(135deg,var(--ucl-mid),var(--ucl-light));border:none;color:white;padding:6px 14px;font-family:'Montserrat',sans-serif;font-weight:700;font-size:12px;cursor:pointer;border-radius:4px;transition:all .2s;text-transform:uppercase;}
...
.user-pill{display:flex;align-items:center;gap:8px;background:rgba(68,114,196,0.15);border:1px solid var(--ucl-light);border-radius:20px;padding:4px 14px 4px 4px;cursor:pointer;}
.user-avatar{width:30px;height:30px;background:linear-gradient(135deg,var(--ucl-mid),var(--ucl-light));border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;flex-shrink:0;font-family:'Bebas Neue',sans-serif;overflow:hidden;}
.user-avatar img{width:100%;height:100%;object-fit:cover;border-radius:50%;}
.user-name{font-size:13px;font-weight:600;color:#EEF4FF;}
...
.card{background:#0D1F3C;border:1px solid var(--ucl-border);border-radius:10px;padding:20px;}
.card-blue{background:linear-gradient(135deg,rgba(27,58,107,0.5),rgba(10,22,40,0.95));border:1px solid var(--ucl-light);}

.btn-primary{background:linear-gradient(135deg,var(--ucl-mid),var(--ucl-light));color:white;border:none;padding:10px 24px;font-family:'Montserrat',sans-serif;font-weight:700;font-size:13px;letter-spacing:1px;cursor:pointer;border-radius:6px;text-transform:uppercase;transition:all .2s;box-shadow:0 4px 14px rgba(68,114,196,0.35);}
.btn-primary:hover{background:linear-gradient(135deg,var(--ucl-light),var(--ucl-bright));transform:translateY(-1px);}
.btn-secondary{background:transparent;color:#C8D8F0;border:1px solid rgba(68,114,196,0.5);padding:10px 24px;font-family:'Montserrat',sans-serif;font-weight:600;font-size:13px;cursor:pointer;border-radius:6px;text-transform:uppercase;transition:all .2s;}
...
.section-title{font-family:'Bebas Neue',sans-serif;font-size:26px;letter-spacing:4px;color:#EEF4FF;display:flex;align-items:center;gap:14px;margin-bottom:18px;}
.section-title::after{content:'';flex:1;height:1px;background:linear-gradient(90deg,var(--ucl-light),transparent);}

.hero{background:linear-gradient(135deg,rgba(27,58,107,0.5) 0%,rgba(10,22,40,0.95) 60%);border:1px solid var(--ucl-light);border-radius:14px;padding:40px 32px;margin-bottom:24px;position:relative;overflow:hidden;}
.hero::before{content:'UCL';position:absolute;right:-20px;bottom:-30px;font-family:'Bebas Neue',sans-serif;font-size:220px;color:rgba(68,114,196,0.05);pointer-events:none;line-height:1;}
.hero::after{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,transparent,var(--ucl-star),var(--ucl-light),transparent);}
.hero-tag{display:inline-block;background:linear-gradient(135deg,var(--ucl-star),#E6A800);color:#0A1628;font-size:11px;font-weight:800;letter-spacing:3px;padding:3px 12px;border-radius:2px;margin-bottom:12px;text-transform:uppercase;}
.hero h1{font-family:'Bebas Neue',sans-serif;font-size:clamp(32px,5vw,68px);letter-spacing:5px;line-height:1;margin-bottom:12px;color:#EEF4FF;}
.hero h1 span{color:var(--ucl-star);}
.hero p{color:#C8D8F0;font-size:15px;max-width:520px;margin-bottom:24px;line-height:1.6;}
.hero-actions{display:flex;gap:12px;flex-wrap:wrap;}

.stats-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:12px;margin-bottom:24px;}
.stat-card{background:linear-gradient(135deg,rgba(27,58,107,0.3),rgba(10,22,40,0.8));border:1px solid var(--ucl-border);border-radius:10px;padding:16px;text-align:center;transition:border-color .2s;}
...
.stat-label{font-size:10px;color:#C8D8F0;letter-spacing:2px;text-transform:uppercase;margin-top:4px;}
...
.player-avatar-circle{width:32px;height:32px;border-radius:50%;background:linear-gradient(135deg,var(--ucl-mid),var(--ucl-light));display:flex;align-items:center;justify-content:center;font-family:'Bebas Neue',sans-serif;font-size:13px;color:white;flex-shrink:0;overflow:hidden;border:2px solid rgba(68,114,196,0.4);}
.player-avatar-circle img{width:100%;height:100%;object-fit:cover;border-radius:50%;}
.player-avatar-circle.sm{width:26px;height:26px;font-size:11px;}
.player-avatar-circle.lg{width:80px;height:80px;font-size:30px;border:3px solid var(--ucl-light);box-shadow:0 0 24px rgba(68,114,196,.5);}

.div-badge{display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:50%;font-weight:800;font-size:13px;flex-shrink:0;font-family:'Montserrat',sans-serif;}
...
.div-guide-card{background:linear-gradient(135deg,rgba(0,100,50,0.22),rgba(10,22,40,0.9));border:1px solid rgba(0,200,80,0.35);border-radius:12px;padding:18px;transition:all .2s;}
...
.div-guide-desc{font-size:12px;color:#C8D8F0;line-height:1.5;}
.div-guide-pts{font-size:11px;color:#FFEA00;font-weight:700;margin-top:5px;letter-spacing:1px;}

/* ===== TABLE — FORCED DARK (no white-on-white) ===== */
...
.lb-table-wrap table{background:#060E1C;}
.lb-table-wrap thead tr{background:#0D1E3A;}
.lb-table-wrap thead th{padding:12px 14px;text-align:left;font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#82B1FF;font-weight:700;white-space:nowrap;background:#0D1E3A;}
.lb-table-wrap tbody tr{border-bottom:1px solid rgba(27,58,107,0.5);background:#060E1C;}
.lb-table-wrap tbody tr:hover{background:#0D1E3A !important;}
.lb-table-wrap tbody tr:last-child{border-bottom:none;}
.lb-table-wrap tbody td{padding:11px 14px;font-size:14px;font-weight:500;vertical-align:middle;color:#D0E4FF;}
.lb-table-wrap tbody tr.top-1{background:linear-gradient(90deg,rgba(255,215,0,0.1),#060E1C) !important;}
.lb-table-wrap tbody tr.top-2{background:linear-gradient(90deg,rgba(192,192,192,0.07),#060E1C) !important;}
.lb-table-wrap tbody tr.top-3{background:linear-gradient(90deg,rgba(205,127,50,0.07),#060E1C) !important;}
...
.lb-tab{flex:1;min-width:70px;background:none;border:none;color:#C8D8F0;padding:8px 12px;font-family:'Montserrat',sans-serif;font-weight:600;font-size:12px;cursor:pointer;border-radius:7px;transition:all .2s;text-transform:uppercase;letter-spacing:1px;}
...
.form-select-sm{background:rgba(27,58,107,.25);border:1px solid var(--ucl-border);border-radius:6px;color:#D0E4FF;padding:10px;font-family:'Montserrat',sans-serif;font-size:12px;cursor:pointer;}
...
.form-input,.form-select{background:rgba(27,58,107,.2);border:1px solid var(--ucl-border);border-radius:6px;color:#D0E4FF;padding:10px 14px;font-family:'Montserrat',sans-serif;font-size:14px;font-weight:500;transition:border-color .2s;width:100%;}
...
.modal{background:#0A1628;border:1px solid var(--ucl-light);border-radius:12px;padding:26px;width:100%;max-width:480px;max-height:90vh;overflow-y:auto;position:relative;box-shadow:0 20px 60px rgba(0,0,0,.7);}
.modal-title{font-family:'Bebas Neue',sans-serif;font-size:26px;letter-spacing:3px;color:#EEF4FF;margin-bottom:18px;}
.modal-close{position:absolute;top:14px;right:14px;background:rgba(68,114,196,.2);border:1px solid var(--ucl-light);color:var(--ucl-bright);width:28px;height:28px;border-radius:50%;font-size:13px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .2s;}
.modal-close:hover{background:var(--ucl-mid);color:white;}

.toast{position:fixed;bottom:80px;right:16px;z-index:9999;background:#0D1E3A;border-left:4px solid var(--ucl-light);border-radius:8px;padding:13px 18px;min-width:240px;max-width:340px;box-shadow:0 8px 30px rgba(0,0,0,.6);transform:translateX(120%);transition:transform .3s;font-weight:600;font-size:14px;color:#D0E4FF;}
.toast.show{transform:translateX(0);}
.toast.success{border-color:var(--green);}
.toast.error{border-color:#FF3D3D;}
.toast.info{border-color:var(--ucl-star);}

.news-ticker{background:linear-gradient(135deg,rgba(27,58,107,0.4),rgba(10,22,40,0.9));border:1px solid var(--ucl-border);border-radius:8px;margin-bottom:18px;overflow:hidden;display:flex;align-items:center;}
.news-label{background:linear-gradient(135deg,var(--ucl-star),#E6A800);color:#0A1628;padding:9px 16px;font-family:'Bebas Neue',sans-serif;font-size:14px;letter-spacing:2px;flex-shrink:0;white-space:nowrap;}
...
.match-card{background:linear-gradient(135deg,rgba(27,58,107,.2),rgba(10,22,40,.8));border:1px solid var(--ucl-border);border-radius:8px;padding:13px 16px;margin-bottom:9px;display:flex;align-items:center;gap:12px;transition:all .2s;}
...
.match-score{font-family:'Bebas Neue',sans-serif;font-size:26px;color:#EEF4FF;text-align:right;flex-shrink:0;}
...
.pending-item{background:linear-gradient(135deg,rgba(27,58,107,.2),rgba(10,22,40,.8));border:1px solid var(--ucl-border);border-radius:8px;padding:13px 16px;margin-bottom:8px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;}
...
.confirm-card{background:linear-gradient(135deg,rgba(68,114,196,.15),rgba(10,22,40,.9));border:1px solid var(--ucl-light);border-radius:10px;padding:16px;margin-bottom:12px;}
...
.profile-info{flex:1;}
.profile-name{font-family:'Bebas Neue',sans-serif;font-size:36px;letter-spacing:3px;line-height:1;color:#EEF4FF;}
.profile-cat{display:inline-block;background:rgba(68,114,196,.25);border:1px solid var(--ucl-light);color:var(--ucl-bright);font-size:11px;font-weight:700;letter-spacing:2px;padding:2px 10px;border-radius:3px;margin-top:5px;text-transform:uppercase;}
.profile-stats{display:grid;grid-template-columns:repeat(auto-fit,minmax(92px,1fr));gap:10px;margin-bottom:18px;}
.p-stat{background:linear-gradient(135deg,rgba(27,58,107,.3),rgba(10,22,40,.8));border:1px solid var(--ucl-border);border-radius:8px;padding:12px;text-align:center;}
...
.promo-tracker{background:linear-gradient(135deg,rgba(0,100,50,.2),rgba(10,22,40,.9));border:1px solid rgba(0,200,80,.3);border-radius:10px;padding:16px;margin-top:16px;}
...
.player-badge{display:inline-flex;align-items:center;gap:5px;padding:5px 11px;border-radius:5px;font-size:10px;font-weight:700;letter-spacing:1px;text-transform:uppercase;border:1px solid;}
...
.admin-tab{background:none;border:1px solid transparent;color:#C8D8F0;padding:7px 13px;font-family:'Montserrat',sans-serif;font-weight:600;font-size:11px;cursor:pointer;border-radius:5px;transition:all .2s;text-transform:uppercase;letter-spacing:1px;}
...
.rc-row{display:grid;grid-template-columns:48px 1fr 90px 110px 80px 70px;align-items:center;padding:12px 28px;border-bottom:1px solid rgba(27,58,107,0.5);gap:8px;background:#060E1C;}
...
.rc-row.rank-1-row{background:linear-gradient(90deg,rgba(255,215,0,.08),#060E1C) !important;}
.rc-row.rank-2-row{background:linear-gradient(90deg,rgba(192,192,192,.06),#060E1C) !important;}
.rc-row.rank-3-row{background:linear-gradient(90deg,rgba(205,127,50,.06),#060E1C) !important;}
...
.rc-avatar{width:30px;height:30px;border-radius:50%;background:linear-gradient(135deg,#1B3A6B,#4472C4);display:flex;align-items:center;justify-content:center;font-family:'Bebas Neue',sans-serif;font-size:13px;color:white;flex-shrink:0;overflow:hidden;}
...
/* Photo preview in admin edit */
.photo-preview-wrap{display:flex;align-items:center;gap:12px;margin-top:8px;}
.photo-preview{width:54px;height:54px;border-radius:50%;background:linear-gradient(135deg,var(--ucl-mid),var(--ucl-light));border:2px solid var(--ucl-light);overflow:hidden;display:flex;align-items:center;justify-content:center;font-family:'Bebas Neue',sans-serif;font-size:20px;color:white;}
  ...
  <div class="lb-table-wrap">
    <table>
      ...
      <tbody id="lbTableBody">
        <tr><td colspan="11" style="text-align:center;padding:30px;color:#4472C4"><div class="spinner" style="margin:0 auto"></div></td></tr>
      </tbody>
    </table>
  </div>
</div>
  ...
  <div id="adminPanelWrap" style="display:none">
    ...
    <div id="adminTab-moderators" style="display:none">
      ...
      <div class="mod-info-box">
        ...
        <p class="text-gray text-sm">Moderators can approve or dispute any pending match result.</p>
      </div>
    ...
    <div id="adminTab-season" style="display:none">
      ...
      <div class="card card-blue" style="max-width:540px;margin-bottom:20px">
        ...
        <div class="match-form">
          <div class="form-group"><label class="form-label">Season Name</label><input class="form-input" id="seasonName" placeholder="Season 1"></div>
  ...
  <div class="modal">
    ...
    <div class="match-form">
      ...
      <div class="form-group">
        ...
        <input class="form-input" id="editPlayerPhoto" placeholder="https://..." oninput="previewPhoto()">
        <div class="photo-preview-wrap">
          <div class="photo-preview" id="photoPreviewBox"><span id="photoPreviewInitial">?</span></div>
          <span class="text-gray text-sm">Preview — shown everywhere next to player name</span>
        </div>
      </div>
      ...
      <div class="form-group"><label class="form-label">ELO Rating (Div 3-1 only)</label><input class="form-input" type="number" id="editEloRating"></div>
...
<script>
// All functions are assigned to window as they are defined inside the async IIFE.
// Stubs ensure onclick never throws before Firebase is ready.
const _pending = {};
function _proxy(name) {
  return function(...args) {
    if (_pending[name]) return _pending[name](...args);
    // Queue it once ready
    document.addEventListener('_appReady', () => _pending[name] && _pending[name](...args), {once:true});
  };
}
['navTo','navBack','setNavActive','setMobActive','openModal','closeModal','previewPhoto',
 'doLogin','doRegister','doLogout','changeMyPassword','switchLbTab','filterLeaderboard',
 'filterMatches','confirmMatch','submitMatchResult','loadSubmitPage','viewProfile','toggleRival',
 'loadMyProfile','openRankingCardPreview','closeRankingPreview','downloadRankingCard',
 'adminLogin','loadAdminPanel','switchAdminTab','adminApprove','adminReject','adminEditPlayer',
 'savePlayerEdit','adminToggleBan','adminToggleMod','adminTogglePOTD','adminTogglePOTW',
 'adminTogglePOTM','adminTogglePOTS','adminEditMatch','saveMatchEdit','adminDeleteMatch',
 'adminDeleteMatchDirect','adminConfirmMatch','filterAdminPlayers','filterModPlayers',
 'confirmSeasonReset','endCurrentSeason','changeAdminPassword','loadModPanel','saveSeasonSettings',
 'autoAwardPOTD','autoAwardPOTW','autoAwardPOTM','autoAwardPOTS','autoAwardMostActive'
].forEach(f => window[f] = _proxy(f));
...
const DIV_RULES = {
  9:{cycle:10,promo:9,  relo:0, name:'Rookie',       useRating:false},
  8:{cycle:10,promo:6,  relo:0, name:'Amateur',      useRating:false},
  7:{cycle:10,promo:9,  relo:2, name:'Regional',     useRating:false},
  6:{cycle:10,promo:11, relo:3, name:'National',     useRating:false},
  5:{cycle:10,promo:12, relo:4, name:'League Two',   useRating:false},
  4:{cycle:10,promo:13, relo:5, name:'League One',   useRating:false},
  3:{useRating:true,             name:'Championship'},
  2:{useRating:true,             name:'Premier'},
  1:{useRating:true,             name:'Elite'},
};
const RATING_PROMO = {3:1500, 2:1800};
const RATING_RELO  = {2:1500, 1:1800};
...
function avatarHtml(name,photo,sizeClass=''){
  ...
  if(photo){
    return `<div class="player-avatar-circle ${sizeClass}"><img src="${photo}" alt="${init}" onerror="this.parentElement.innerHTML='${init}'"></div>`;
  }
  return `<div class="player-avatar-circle ${sizeClass}">${init}</div>`;
}
...
function previewPhoto(){
  const url=($('editPlayerPhoto')?.value||'').trim();
  const box=$('photoPreviewBox');
  const nameVal=($('editPlayerName')?.value||'?')[0].toUpperCase();
  if(!box)return;
  if(url){
    box.innerHTML=`<img src="${url}" alt="preview" onerror="this.parentElement.innerHTML='${nameVal}'" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`;
  } else {
    box.innerHTML=`<span>${nameVal}</span>`;
  }
}
...
async function doRegister(){
  ...
  try{
    ...
    await addDoc(collection(db,'players'),{
      ...
      cycleMP:0,cyclePts:0,eloRating:DEFAULT_ELO,eloChange:0,photoUrl:'',
      isPOTD:false,isPOTW:false,isPOTM:false,isPOTS:false,createdAt:serverTimestamp()
    });
...
function updateHeaderAuth(){
  ...
  if(S.user){
    ...
    const init=(S.user.name||'?')[0].toUpperCase();
    const avatarInner=photo?`<img src="${photo}" alt="${init}" onerror="this.style.display='none'" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`:`${init}`;
    el.innerHTML=`
...
async function loadHome(){
  try{
    ...
    const ranked=S.players.map(p=>{
      const pm=S.matches.filter(m=>(m.playerAId===p.id||m.playerBId===p.id)&&m.status==='confirmed');
      ...
      const rating=rules.useRating?(p.eloRating||DEFAULT_ELO):Math.max(0,Math.round(w*10+d*5+l*(-5)+(gf-ga)+cs*2));
      return{...p,rating,tw:Math.round(w),td:d,tl:Math.round(l)};
    }).sort((a,b)=>b.rating-a.rating);
    const top5=ranked.slice(0,5);
    $('homeTopPlayers').innerHTML=top5.length?top5.map((p,i)=>`
      <div class="pending-item" style="cursor:pointer" onclick="viewProfile('${p.id}')">
        ...
        <div class="pending-info">
          <div class="pending-name" style="display:flex;align-items:center;gap:8px">
            ${avatarHtml(p.name,p.photoUrl||'','sm')}
            ${p.name} <span class="${catClass(p.category||'Main')}" style="font-size:11px">${p.category||'Main'}</span>
          </div>
          <div class="pending-meta">${p.tw}W ${p.td}D ${p.tl}L</div>
        </div>
    ...
    $('homeRecentMatches').innerHTML=recent.length?recent.map(m=>{
      ...
      const pA=S.players.find(p=>p.id===m.playerAId)||{};
      const pB=S.players.find(p=>p.id===m.playerBId)||{};
      return `<div class="match-card${hi?' high-score':''}">
        <div class="match-info">
          <div class="match-vs" style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
            ${avatarHtml(m.playerAName,pA.photoUrl||'','sm')}
            <span class="player-link" onclick="viewProfile('${m.playerAId}')">${m.playerAName}</span>
            <span class="text-gray">vs</span>
            ${avatarHtml(m.playerBName,pB.photoUrl||'','sm')}
            <span class="player-link" onclick="viewProfile('${m.playerBId}')">${m.playerBName}</span>
    ...
    const divInfo=[
      ...
      {d:4,name:'League One',  promo:'13 pts → Enter ELO Div 3', relo:'Relegated if 5 pts or less in cycle',tier:'low'},
      {d:3,name:'Championship',promo:'ELO 1500 → Div 2',         relo:'No demotion (ELO floor 1200)',tier:'top'},
      {d:2,name:'Premier',     promo:'ELO 1800 → Div 1',         relo:'Below 1500 ELO → Div 3',tier:'top'},
      {d:1,name:'Elite',       promo:'Top Division',             relo:'Below 1800 ELO → Div 2',tier:'top'},
    ];
    $('divisionGuide').innerHTML=divInfo.map(di=>`
      <div class="div-guide-card" style="${di.tier==='top'?'border-color:rgba(245,197,24,0.4);background:linear-gradient(135deg,rgba(245,197,24,0.08),rgba(10,22,40,0.9))':''}">
        <div style="display:flex;align-items:center;gap:10px">${divBadge(di.d)}<div class="div-guide-name">${di.name}</div></div>
        <div class="div-guide-pts">&#8679; ${di.promo}</div>
        <div class="div-guide-desc">${di.relo}</div>
        ${di.tier==='top'?'<div style="font-size:10px;color:#F5C518;margin-top:4px;font-weight:700;letter-spacing:1px">ELO RATING SYSTEM</div>':''}
      </div>`).join('');
...
async function loadLeaderboard(tab='overall'){
  S.lbTab=tab;
  $('lbTableBody').innerHTML='<tr><td colspan="11" style="text-align:center;padding:30px;color:#4472C4"><div class="spinner" style="margin:0 auto"></div></td></tr>';
  ...
  if(seasonSel){
    ...
    }else{seasonSel.style.display='none';}
  }
  try{
    const [pSnap,mSnap]=await Promise.all([
      getDocs(collection(db,'players')),
    ...
    S.matches=mSnap.docs.map(d=>({id:d.id,...d.data()}))
      .sort((a,b)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0));
    const now=Date.now();
    let seasonStartMs=0,seasonEndMs=now,matchFilter=null;
    if(tab==='daily'){matchFilter=m=>now-(m.createdAt?.toDate?.()?.getTime()||0)<86400000;}
      ...
      if(selVal==='current'){
        try{const ss=await getDoc(doc(db,'settings','season'));if(ss.exists()&&ss.data().startDate)seasonStartMs=new Date(ss.data().startDate).getTime();if(ss.exists()&&ss.data().endDate)seasonEndMs=new Date(ss.data().endDate).getTime();}catch(e){console.error(e);}
      }else{
        try{const ps=await getDoc(doc(db,'pastSeasons',selVal));if(ps.exists()){if(ps.data().startDate)seasonStartMs=new Date(ps.data().startDate).getTime();if(ps.data().endDate)seasonEndMs=new Date(ps.data().endDate).getTime();}}catch(e){console.error(e);}
      }
    ...
    let players=S.players.map(p=>{
      ...
      const div=p.division||9,rules=DIV_RULES[div]||DIV_RULES[9];
      const rating=rules.useRating?(p.eloRating||DEFAULT_ELO):Math.max(0,Math.round(w*10+d*5+l*(-5)+gd+cs*2));
    ...
    $('lbTableBody').innerHTML='<tr><td colspan="11" style="text-align:center;color:#FF5555;padding:20px">Error loading</td></tr>';
    console.error(e);
  }
}
...
function renderLbTable(players){
  ...
  tbody.innerHTML=filtered.map((p,i)=>{
    ...
    let cycleCell='';
    if(isElo){
      const elo=p.eloRating||DEFAULT_ELO,next=RATING_PROMO[p.division];
      if(next){
        const pct=Math.min(100,Math.round(Math.max(0,elo-DEFAULT_ELO)/(next-DEFAULT_ELO)*100));
        cycleCell=`<div style="font-size:10px;color:#F5C518">${elo} ELO</div><div style="background:rgba(255,255,255,.06);border-radius:3px;height:4px;width:48px;margin-top:3px;overflow:hidden"><div style="height:100%;width:${pct}%;background:linear-gradient(90deg,#F5C518,#FFD700);border-radius:3px"></div></div>`;
      }else{cycleCell=`<div style="font-size:10px;color:#F5C518">${elo} ELO</div><div style="font-size:9px;color:#4472C4">Elite</div>`;}
    }else{
      const pct=Math.min(100,Math.round((p.cyclePts||0)/(rules.promo||1)*100));
      cycleCell=`<div style="font-size:10px;color:#4472C4">${p.cycleMP||0}/10</div><div style="background:rgba(255,255,255,.06);border-radius:3px;height:4px;width:48px;margin-top:3px;overflow:hidden"><div style="height:100%;width:${pct}%;background:linear-gradient(90deg,#00A550,#00E676);border-radius:3px"></div></div>`;
    }
    const rCol=r===1?'#FFD700':r===2?'#C0C0C0':r===3?'#CD7F32':'#4472C4';
    return `<tr class="${rc}">
      <td><span style="font-family:'Bebas Neue',sans-serif;font-size:24px;color:${rCol}">${r}</span></td>
      <td><span class="player-link" onclick="viewProfile('${p.id}')">
        ${avatarHtml(p.name,p.photoUrl||'','sm')}
        <span style="color:#D0E4FF;font-weight:600">${p.name}</span>&nbsp;<span class="${catClass(p.category||'Main')}" style="font-size:10px;font-weight:700">${p.category||'Main'}</span>
        ${p.isModerator?'&nbsp;<span class="mod-badge-hdr">MOD</span>':''}
      </span></td>
      <td>${divBadge(p.division)}</td>
      <td><span style="font-family:'Bebas Neue',sans-serif;font-size:20px;color:#F5C518">${p.rating}</span></td>
      <td><div class="wdl-row">
      ...
      <td><div class="winrate-wrap"><div class="winrate-bar"><div class="winrate-fill" style="width:${p.twr}%"></div></div><span class="winrate-pct">${p.twr}%</span></div></td>
...
function buildRankingCard(){
  ...
  const rows=top10.map((p,i)=>{
    ...
    const avatarContent=photo?`<img src="${photo}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;" onerror="this.style.display='none'">`:`${p.name[0].toUpperCase()}`;
    return `<div class="rc-row ${rc}">
      <div class="rc-rank ${rankCls}">${r}</div>
      <div class="rc-player-info"><div class="rc-avatar">${avatarContent}</div><div><div class="rc-name">${p.name}</div><div class="rc-div-small">DIV ${p.division||9} - ${divNames[p.division||9]||''} | ${p.category||'Main'}</div></div></div>
      ...
      <div style="display:flex;gap:5px;align-items:center"><div style="background:rgba(255,255,255,.06);border-radius:4px;height:5px;width:58px;overflow:hidden"><div style="height:100%;width:${p.twr}%;background:linear-gradient(90deg,#1B3A6B,#4472C4);border-radius:4px"></div></div><span style="font-family:'Bebas Neue',sans-serif;font-size:13px;color:#4472C4">${p.twr}%</span></div>
      <div class="rc-winpct" style="color:${p.tgd>=0?'#00E676':'#FF5555'}">${p.tgd>=0?'+':''}${p.tgd}</div>
    </div>`;
  }).join('');
  $('rcCardContainer').innerHTML=`<div class="rc-card" id="theRankingCard">
    <div class="rc-header"><div class="rc-header-top"><div><div class="rc-title">LLFC EFOOTBALL DIVISION</div><div class="rc-subtitle">PLAYER RANKING - TOP 10</div></div><div class="rc-period">${labels[S.lbTab]||'Overall'}</div></div></div>
    <div class="rc-body"><div class="rc-row rc-header-row"><span>Rank</span><span>Player</span><span>Rating</span><span>W/D/L</span><span>Win%</span><span>GD</span></div>${rows}</div>
    <div class="rc-footer"><div class="rc-footer-brand">LLFC EFOOTBALL DIVISION SYSTEM</div><div class="rc-footer-date">GENERATED: ${now}</div></div>
  </div>`;
}
...
async function loadMatches(){
  try{
    ...
    if(!S.players.length){const pSnap=await getDocs(collection(db,'players'));S.players=pSnap.docs.map(d=>({id:d.id,...d.data()})).filter(p=>p.status==='active');}
    renderMatchesList(S.matches);renderPendingConfirms();
...
function renderMatchesList(matches){
  ...
  const photoMap={};S.players.forEach(p=>photoMap[p.id]=p.photoUrl||'');
  el.innerHTML=f.map(m=>{
    const hi=isHigh(m.scoreA,m.scoreB),x2=m.is2x;
    return `<div class="match-card${hi?' high-score':''}">
      <div class="match-info">
        <div class="match-vs" style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
          ${avatarHtml(m.playerAName,photoMap[m.playerAId]||'','sm')}
          <span class="player-link" onclick="viewProfile('${m.playerAId}')">${m.playerAName}</span>
          <span class="text-gray">vs</span>
          ${avatarHtml(m.playerBName,photoMap[m.playerBId]||'','sm')}
          <span class="player-link" onclick="viewProfile('${m.playerBId}')">${m.playerBName}</span>
          ${hi?' <span class="high-score-badge">GOAL FEST</span>':''}${x2?' <span class="x2-badge">2X</span>':''}
        </div>
...
function renderPendingConfirms(){
  ...
  list.innerHTML=pending.map(m=>`
    <div class="confirm-card">
      ${isMod&&m.playerBId!==S.user.id?'<div style="font-size:11px;color:#CE93D8;font-weight:700;margin-bottom:8px">MODERATOR REVIEW</div>':''}
...
async function applyMatchStats(m){
  ...
  const multA=getRatingMultiplier(aCat,bCat,getRC(m.scoreA,m.scoreB,'A'));
  const multB=getRatingMultiplier(bCat,aCat,getRC(m.scoreA,m.scoreB,'B'));
  await updateDoc(doc(db,'matches',m.id||'_'),{playerACat:aCat,playerBCat:bCat,is2x:multA>1||multB>1}).catch(()=>{});
  async function upd(pid,pData,side,oppData){
    ...
    const div=pData.division||9,rules=DIV_RULES[div]||DIV_RULES[9];
    ...
    let updates={
      ...
      cleanSheets:(pData.cleanSheets||0)+(isCS?1:0),form,
    };
    if(rules.useRating){
      const eloConf=ELO_CHANGE[div]||{win:30,loss:30};
      const myRating=pData.eloRating||DEFAULT_ELO;
      let ratingChange=0;
      if(result==='W')ratingChange=Math.round(eloConf.win*mult);
      else if(result==='L')ratingChange=-Math.round(eloConf.loss*mult);
      const floor=div===3?DEFAULT_ELO:0;
      const newRating=Math.max(floor,myRating+ratingChange);
      updates.eloRating=newRating;updates.eloChange=ratingChange;
      let newDiv=div,action=null;
      if(div===3&&newRating>=RATING_PROMO[3]){newDiv=2;action='promote';updates.eloRating=DEFAULT_ELO;}
      else if(div===2&&newRating>=RATING_PROMO[2]){newDiv=1;action='promote';}
      else if(div===2&&newRating<RATING_RELO[2]){newDiv=3;action='relegate';updates.eloRating=DEFAULT_ELO;}
      else if(div===1&&newRating<RATING_RELO[1]){newDiv=2;action='relegate';updates.eloRating=RATING_PROMO[2]-10;}
      if(action){updates.division=newDiv;updates.highestDivision=Math.min(newDiv,pData.highestDivision||9);
        if(action==='promote')setTimeout(()=>T(pData.name+' promoted to Division '+newDiv+'!','success'),100);
        else setTimeout(()=>T(pData.name+' relegated to Division '+newDiv+'.','info'),100);}
    }else{
      const cyclePtsGain=result==='W'?3:result==='D'?1:0;
      const newCycleMP=(pData.cycleMP||0)+1,newCyclePts=(pData.cyclePts||0)+cyclePtsGain;
      updates.cycleMP=newCycleMP;updates.cyclePts=newCyclePts;
      let newDiv=div,action=null;
      if(newCyclePts>=rules.promo&&div>1){
        newDiv=div-1;action='promote';updates.cycleMP=0;updates.cyclePts=0;
        if(newDiv<=3){updates.eloRating=DEFAULT_ELO;updates.eloChange=0;}
      }else if(newCycleMP>=rules.cycle){
        if(rules.relo>0&&newCyclePts<=rules.relo&&div<9){newDiv=div+1;action='relegate';}
  ...
  await Promise.all([upd(m.playerAId,aData,'A',bData),upd(m.playerBId,bData,'B',aData)]);
}
...
async function submitMatchResult(){
  ...
  const oppId=$('submitOpponent').value,sA=parseInt($('scoreA').value),sB=parseInt($('scoreB').value),matchDate=$('matchDate').value;
  ...
  const btn=document.querySelector('#submitForm .btn-primary');if(btn)btn.disabled=true;
  try{
    await addDoc(collection(db,'matches'),{
      ...
      scoreA:sA,scoreB:sB,status:'pending',matchDate,createdAt:serverTimestamp(),submittedBy:S.user.id
    });
    T(isHigh(sA,sB)?`GOAL FEST! ${sA+sB} goals submitted. Waiting for ${opp.name}.`:`Result submitted. Waiting for ${opp.name} to confirm.`,'success');
    $('scoreA').value='';$('scoreB').value='';$('submitOpponent').value='';S.matches=[];
  }catch(e){T('Submit failed: '+e.message,'error');}
  finally{if(btn)btn.disabled=false;}
}

async function loadModPanel(){
  if(!S.user||!S.user.isModerator){$('modPendingMatches').innerHTML='<div class="empty-state"><div class="empty-text">Moderator access required.</div></div>';return;}
  const el=$('modPendingMatches');
  el.innerHTML='<div class="loading-spinner"><div class="spinner"></div></div>';
  try{
    const mSnap=await getDocs(query(collection(db,'matches'),where('status','==','pending')));
    const pending=mSnap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0));
    ...
    el.innerHTML=pending.map(m=>`
      <div class="confirm-card">
        ...
        <div class="confirm-vs">
          <div class="confirm-player"><div class="confirm-player-name">${m.playerAName}</div><div class="text-gray text-sm">${m.playerACat||'Main'}</div></div>
          <div class="confirm-score-display">${m.scoreA}-${m.scoreB}</div>
          <div class="confirm-player"><div class="confirm-player-name">${m.playerBName}</div><div class="text-gray text-sm">${m.playerBCat||'Main'}</div></div>
        </div>
        <div class="text-gray text-sm mb-8">${fmtDate(m.createdAt)} - ${timeAgo(m.createdAt)}</div>
        <div style="display:flex;gap:8px;flex-wrap:wrap">
  ...
  }catch(e){el.innerHTML=`<div class="empty-state"><div class="empty-text">Error: ${e.message}</div></div>`;console.error(e);}
}
...
async function viewProfile(playerId){
  ...
  try{
    const [snap,mSnap,pSnap]=await Promise.all([getDoc(doc(db,'players',playerId)),getDocs(collection(db,'matches')),getDocs(collection(db,'players'))]);
    if(!snap.exists()){$('profileContent').innerHTML='<div class="empty-state"><div class="empty-text">Player not found</div></div>';return;}
    const p={id:snap.id,...snap.data()};
    const allM=mSnap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0));
    const allPlayers=pSnap.docs.map(d=>({id:d.id,...d.data()}));
    const playerCatMap={},playerPhotoMap={};
    allPlayers.forEach(pp=>{playerCatMap[pp.id]=pp.category||'Main';playerPhotoMap[pp.id]=pp.photoUrl||'';});
    ...
    const total=myConfirmed.length,wr=total>0?Math.round((w/(w+d+l||1))*100):0,gd=gf-ga;
    const div=p.division||9,rules=DIV_RULES[div]||DIV_RULES[9];
    const displayRating=rules.useRating?(p.eloRating||DEFAULT_ELO):Math.max(0,Math.round(w*10+d*5+l*(-5)+gd+cs*2));
    const isEloDiv=rules.useRating,cmp=p.cycleMP||0,cpts=p.cyclePts||0,eloRating=p.eloRating||DEFAULT_ELO,photo=p.photoUrl||'';
    let promoHTML='';
    if(isEloDiv){
      const nextPromo=RATING_PROMO[div];
      if(nextPromo){
        const pct=Math.min(100,Math.round(Math.max(0,eloRating-DEFAULT_ELO)/(nextPromo-DEFAULT_ELO)*100));
        const reloText=div===2?`Below ${RATING_RELO[2]} ELO → relegate to Div 3`:div===1?`Below ${RATING_RELO[1]} ELO → relegate to Div 2`:'';
        promoHTML=`<div class="promo-tracker"><div class="promo-title">ELO Rating - Div ${div} (${rules.name})</div>
          ...
          ${reloText?`<div class="promo-cycle-info" style="color:#FF5555;margin-top:4px">${reloText}</div>`:''}
        </div>`;
      }else{
        promoHTML=`<div class="promo-tracker"><div class="promo-title">ELO Rating - Div 1 Elite</div>
          ...
          <div class="promo-cycle-info" style="color:#F5C518">Win: +20 | Loss: -40</div>
          <div class="promo-cycle-info" style="color:#FF5555;margin-top:4px">Below 1800 ELO → relegate to Div 2</div>
        </div>`;
      }
    }else{
      const pct=Math.min(100,Math.round(cpts/(rules.promo||1)*100));
      promoHTML=`<div class="promo-tracker"><div class="promo-title">Division ${div} Cycle Progress</div>
        ...
        ${cmp>=rules.cycle?'<div class="promo-cycle-info" style="color:#00E676">Cycle complete</div>':`<div class="promo-cycle-info">${Math.max(0,rules.cycle-cmp)} matches left</div>`}
      </div>`;
    }
    const form=(p.form||[]).slice(-5);
    ...
    const potdC=p.potdCount||0;if(p.isPOTD||potdC>0){if(potdC>=5)badges.push({t:'POTD Legend x'+potdC,c:'badge-diamond',icon:'♦'});else if(potdC>=3)badges.push({t:'POTD Elite x'+potdC,c:'badge-star',icon:'★'});else if(potdC>=2)badges.push({t:'2x Player of Day',c:'badge-gold',icon:'☀'});else badges.push({t:'Player of the Day',c:'badge-gold',icon:'☀'});}
    const potwC=p.potwCount||0;if(p.isPOTW||potwC>0){if(potwC>=3)badges.push({t:'POTW Legend x'+potwC,c:'badge-diamond',icon:'♦'});else if(potwC>=2)badges.push({t:'2x Player of Week',c:'badge-purple',icon:'◉'});else badges.push({t:'Player of the Week',c:'badge-purple',icon:'◉'});}
    const potmC=p.potmCount||0;if(p.isPOTM||potmC>0){if(potmC>=3)badges.push({t:'POTM Legend x'+potmC,c:'badge-diamond',icon:'♦'});else if(potmC>=2)badges.push({t:'2x Player of Month',c:'badge-blue',icon:'◑'});else badges.push({t:'Player of the Month',c:'badge-blue',icon:'◑'});}
    const potsC=p.potsCount||0;if(p.isPOTS||potsC>0){if(potsC>=2)badges.push({t:'Season Champion x'+potsC,c:'badge-diamond',icon:'♛'});else badges.push({t:'Player of the Season',c:'badge-star',icon:'♛'});}
    if(p.isMostActiveD)badges.push({t:'Most Active Today',c:'badge-green',icon:'▶'});
    if(p.isMostActiveW)badges.push({t:'Most Active Week',c:'badge-green',icon:'▶▶'});
    if(p.isMostActiveM)badges.push({t:'Most Active Month',c:'badge-blue',icon:'▶▶▶'});
    if(p.isModerator)badges.push({t:'Moderator',c:'badge-blue',icon:'⚑'});
    $('profileContent').innerHTML=`
      <div class="profile-header">
        ${avatarHtml(p.name,photo,'lg')}
      ...
      ${promoHTML}
      <div class="section-title mt-16">Full Match History (${myConfirmed.length} matches)</div>
      ${myConfirmed.length?myConfirmed.map(m=>{
        const side=m.playerAId===playerId?'A':'B';
        const res=getRC(m.scoreA,m.scoreB,side);
        ...
        const myG=side==='A'?m.scoreA:m.scoreB,opG=side==='A'?m.scoreB:m.scoreA;
        const hi=isHigh(m.scoreA,m.scoreB),x2=m.is2x;
        const oppPhoto=playerPhotoMap[oppId2]||'';
        return `<div class="match-card${hi?' high-score':''}">
          <div class="match-result-badge result-${res}">${res}</div>
          <div class="match-info">
            <div class="match-vs" style="display:flex;align-items:center;gap:6px">
              ${avatarHtml(oppName,oppPhoto,'sm')}
              vs <span class="player-link" onclick="viewProfile('${oppId2}')">${oppName}</span>
              ${hi?' <span class="high-score-badge">GOAL FEST</span>':''}${x2?' <span class="x2-badge">2X</span>':''}
            </div>
            <div class="match-meta">${fmtDate(m.createdAt||m.matchDate)} - ${statusBadge(m.status)}</div>
          </div>
...
async function loadMyProfile(){
  if(!S.user){$('myProfileContent').innerHTML='<div class="empty-state"><div class="empty-icon">&#128274;</div><div class="empty-text">Not logged in</div><button class="btn-primary mt-12" onclick="openModal(\'loginModal\')">Login</button></div>';return;}
  try{const snap=await getDoc(doc(db,'players',S.user.id));if(snap.exists())S.user={...S.user,...snap.data()};}catch{}
  $('myProfileContent').innerHTML='<div class="loading-spinner"><div class="spinner"></div></div>';
  const tmp=S.pageHistory;S.pageHistory=[];
...
async function adminLogin(){
  ...
  if(pw===stored){$('adminLoginWrap').style.display='none';$('adminPanelWrap').style.display='block';loadAdminPanel('pending');T('Admin access granted','success');}
...
async function loadAdminPanel(tab){
  ...
  if(tab==='adminMatches'){
    ...
    const matches=snap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0));
  ...
  if(tab==='moderators'){const snap=await getDocs(collection(db,'players'));S.adminPlayers=snap.docs.map(d=>({id:d.id,...d.data()})).filter(p=>p.status==='active');renderModPlayers(S.adminPlayers);}
  if(tab==='season'){loadSeasonInfo();}
}
...
function renderAdminPlayers(players){
  ...
  const getR=p=>{const rules=DIV_RULES[p.division||9]||DIV_RULES[9];return rules.useRating?(p.eloRating||DEFAULT_ELO)+' ELO':Math.max(0,Math.round((p.wins||0)*10+(p.draws||0)*5+(p.losses||0)*(-5)+((p.goalsFor||0)-(p.goalsAgainst||0))+(p.cleanSheets||0)*2))+'';};
  $('adminPlayersList').innerHTML=filtered.length?filtered.map(p=>`
    <div class="pending-item">
      ${avatarHtml(p.name,p.photoUrl||'','sm')}${divBadge(p.division)}
      <div class="pending-info">
        ...
        <div class="pending-meta">Div${p.division||9} | Rating:${getR(p)} | ${p.wins||0}W ${p.draws||0}D ${p.losses||0}L | Cycle:${p.cycleMP||0}/10 (${p.cyclePts||0}pts) | ELO:${p.eloRating||DEFAULT_ELO} | ${statusBadge(p.status||'active')}</div>
      </div>
      <div style="display:flex;gap:4px;flex-wrap:wrap">
        <button class="btn-sm btn-edit" onclick="adminEditPlayer('${p.id}')">Edit</button>
        <button class="btn-sm btn-ban" onclick="adminToggleBan('${p.id}','${p.name}','${p.status||'active'}')">${p.status==='banned'?'Unban':'Ban'}</button>
...
function renderModPlayers(players){
  ...
  $('modPlayersList').innerHTML=filtered.length?filtered.map(p=>`
    <div class="pending-item">
      ...
      <button class="btn-sm ${p.isModerator?'btn-reject':'btn-mod'}" onclick="adminToggleMod('${p.id}','${p.name}',${!!p.isModerator})">${p.isModerator?'Remove Mod':'Give Mod'}</button>
...
async function adminToggleMod(id,name,isMod){
  const nv=!isMod;await updateDoc(doc(db,'players',id),{isModerator:nv});
...
async function adminReject(id){if(!confirm('Reject and delete?'))return;await deleteDoc(doc(db,'players',id));T('Rejected','info');loadAdminPanel('pending');}

async function adminEditPlayer(id){
  const snap=await getDoc(doc(db,'players',id));if(!snap.exists())return T('Player not found','error');
  const p=snap.data();
  $('editPlayerId').value=id;$('editPlayerName').value=p.name||'';$('editPlayerPhoto').value=p.photoUrl||'';
  $('editDivision').value=p.division||9;$('editCategory').value=p.category||'Main';
  $('editWins').value=p.wins||0;$('editDraws').value=p.draws||0;$('editLosses').value=p.losses||0;
  $('editGF').value=p.goalsFor||0;$('editGA').value=p.goalsAgainst||0;$('editCS').value=p.cleanSheets||0;
  $('editCycleMP').value=p.cycleMP||0;$('editCyclePts').value=p.cyclePts||0;
  $('editEloRating').value=p.eloRating||DEFAULT_ELO;$('editStatus').value=p.status||'active';$('editPlayerPw').value='';
  previewPhoto();openModal('editPlayerModal');
}

async function savePlayerEdit(){
  const id=$('editPlayerId').value,div=parseInt($('editDivision').value);
  const newName=($('editPlayerName').value||'').trim(),photoUrl=($('editPlayerPhoto').value||'').trim();
  ...
  const eloRating=parseInt($('editEloRating').value)||DEFAULT_ELO;
  const status=$('editStatus').value,newPw=($('editPlayerPw').value||'').trim();
  ...
  try{
    ...
    const conflict=existing.docs.find(d=>d.id!==id);if(conflict)return T('Name already taken','error');
    const updates={name:newName,photoUrl,division:div,category:cat,wins,draws,losses,goalsFor:gf,goalsAgainst:ga,cleanSheets:cs,cycleMP:cmp,cyclePts:cpts,eloRating,status};
    if(newPw)updates.password=newPw;
    await updateDoc(doc(db,'players',id),updates);
    if(S.user&&S.user.id===id){S.user={...S.user,...updates};updateHeaderAuth();}
...
function adminEditMatch(id,nA,nB,sA,sB,status){$('editMatchId').value=id;$('editMatchInfo').textContent=`${nA} vs ${nB}`;$('editScoreA').value=sA;$('editScoreB').value=sB;$('editMatchStatus').value=status;openModal('editMatchModal');}
async function adminConfirmMatch(matchId){
  const mRef=doc(db,'matches',matchId),mSnap=await getDoc(mRef);if(!mSnap.exists())return;
  await updateDoc(mRef,{status:'confirmed',confirmedAt:serverTimestamp()});
  await applyMatchStats({...mSnap.data(),id:matchId});
  T('Match confirmed by admin','success');S.matches=[];loadAdminPanel('adminMatches');
}
async function saveMatchEdit(){
  const id=$('editMatchId').value,sA=parseInt($('editScoreA').value),sB=parseInt($('editScoreB').value),status=$('editMatchStatus').value;
  try{
    const mRef=doc(db,'matches',id),old=await getDoc(mRef);
    await updateDoc(mRef,{scoreA:sA,scoreB:sB,status});
    if(status==='confirmed'&&old.data().status!=='confirmed')await applyMatchStats({...old.data(),scoreA:sA,scoreB:sB,id});
    T('Match updated','success');closeModal('editMatchModal');S.matches=[];loadAdminPanel('adminMatches');
  }catch(e){T('Error: '+e.message,'error');}
}
async function adminDeleteMatch(){const id=$('editMatchId').value;if(!confirm('Delete this match permanently?'))return;await deleteDoc(doc(db,'matches',id));T('Match deleted','info');closeModal('editMatchModal');S.matches=[];loadAdminPanel('adminMatches');}
async function adminDeleteMatchDirect(id){if(!confirm('Delete this match?'))return;await deleteDoc(doc(db,'matches',id));T('Match deleted.','info');S.matches=[];loadAdminPanel('adminMatches');}

async function saveSeasonSettings(){
  const name=$('seasonName')?.value||'Season 1',startDate=$('seasonStart').value,endDate=$('seasonEnd').value;
  if(!startDate||!endDate)return T('Set start and end dates','error');
  try{await setDoc(doc(db,'settings','season'),{name,startDate,endDate,updatedAt:serverTimestamp()},{merge:true});T('Season settings saved','success');loadSeasonInfo();}
  catch(e){T('Error: '+e.message,'error');}
}
async function loadSeasonInfo(){
  try{
    const snap=await getDoc(doc(db,'settings','season'));
    const el=$('currentSeasonInfo');if(!el)return;
    if(snap.exists()){
      const d=snap.data(),end=d.endDate?new Date(d.endDate):null;
      const daysLeft=end?Math.max(0,Math.ceil((end-new Date())/86400000)):null;
      el.innerHTML=`<div style="font-weight:700;color:var(--ucl-star)">${d.name||'No season set'}</div><div>Start: ${d.startDate||'-'} | End: ${d.endDate||'-'}</div>${daysLeft!==null?`<div style="color:#00E676;margin-top:4px">${daysLeft} days remaining</div>`:''}`;
      if(d.name)$('seasonName').value=d.name;if(d.startDate)$('seasonStart').value=d.startDate;if(d.endDate)$('seasonEnd').value=d.endDate;
    }else{el.innerHTML='<div style="color:#4472C4">No active season configured</div>';}
    await loadPastSeasons();
  }catch(e){console.error(e);}
}
async function loadPastSeasons(){
  const el=$('pastSeasonsList');if(!el)return;
  try{
    const snap=await getDocs(collection(db,'pastSeasons'));
    const seasons=snap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>(b.endedAt?.seconds||0)-(a.endedAt?.seconds||0));
    if(!seasons.length){el.innerHTML='<div class="text-gray text-sm">No past seasons yet</div>';return;}
    el.innerHTML=seasons.map(s=>`<div style="padding:10px;border:1px solid var(--ucl-border);border-radius:6px;margin-bottom:8px"><div style="font-family:'Bebas Neue',sans-serif;font-size:16px;letter-spacing:2px;color:var(--ucl-star)">${s.name||'Season'}</div><div class="text-gray text-sm">${s.startDate||''} to ${s.endDate||''}</div>${s.topPlayer?`<div style="font-size:12px;color:#00E676;margin-top:4px">Top: ${s.topPlayer}</div>`:''}</div>`).join('');
  }catch(e){el.innerHTML='<div class="text-gray text-sm">Error loading</div>';}
}
async function endCurrentSeason(){
  ...
  try{
    ...
    const sorted=players.map(p=>{const w=p.wins||0,d2=p.draws||0,l=p.losses||0;const rating=(DIV_RULES[p.division||9]||{}).useRating?(p.eloRating||DEFAULT_ELO):Math.max(0,w*10+d2*5+l*(-5)+((p.goalsFor||0)-(p.goalsAgainst||0))+(p.cleanSheets||0)*2);return{...p,rating};}).sort((a,b)=>b.rating-a.rating);
    const topPlayer=sorted[0]?.name||'-',potd=players.find(p=>p.isPOTD)?.name||'-',potw=players.find(p=>p.isPOTW)?.name||'-',potm=players.find(p=>p.isPOTM)?.name||'-';
    await addDoc(collection(db,'pastSeasons'),{name:seasonData.name||'Season',startDate:seasonData.startDate||'',endDate:seasonData.endDate||'',topPlayer,potd,potw,potm,playerCount:players.length,matchCount:matches.length,endedAt:serverTimestamp()});
    await Promise.all(players.map(p=>updateDoc(doc(db,'players',p.id),{cycleMP:0,cyclePts:0,wins:0,draws:0,losses:0,goalsFor:0,goalsAgainst:0,cleanSheets:0,form:[],isPOTD:false,isPOTW:false,isPOTM:false,isPOTS:false})));
    await Promise.all(mSnap.docs.map(d=>deleteDoc(doc(db,'matches',d.id))));
    S.players=[];S.matches=[];T('Season ended and archived!','success');loadSeasonInfo();
...
async function autoAwardPOTD(){const winner=await getTopPlayerInWindow(86400000);if(!winner)return T('No matches in last 24h','info');await grantAward(winner.id,winner.name,'POTD','isPOTD','potdCount');await postSystemNews(`PLAYER OF THE DAY: ${winner.name} claimed the daily top spot!`);$('awardResult').textContent='POTD: '+winner.name;T('POTD awarded to '+winner.name,'success');}
async function autoAwardPOTW(){const winner=await getTopPlayerInWindow(604800000);if(!winner)return T('No matches this week','info');await grantAward(winner.id,winner.name,'POTW','isPOTW','potwCount');await postSystemNews(`PLAYER OF THE WEEK: ${winner.name} dominated the weekly rankings!`);$('awardResult').textContent='POTW: '+winner.name;T('POTW awarded to '+winner.name,'success');}
async function autoAwardPOTM(){const winner=await getTopPlayerInWindow(2592000000);if(!winner)return T('No matches this month','info');await grantAward(winner.id,winner.name,'POTM','isPOTM','potmCount');await postSystemNews(`PLAYER OF THE MONTH: ${winner.name} is the standout performer!`);$('awardResult').textContent='POTM: '+winner.name;T('POTM awarded to '+winner.name,'success');}
async function autoAwardPOTS(){const winner=await getTopPlayerInWindow(Infinity);if(!winner)return T('No matches found','info');await grantAward(winner.id,winner.name,'POTS','isPOTS','potsCount');await postSystemNews(`PLAYER OF THE SEASON: ${winner.name} is crowned the season champion!`);$('awardResult').textContent='POTS: '+winner.name;T('POTS awarded to '+winner.name,'success');}
async function autoAwardMostActive(){
  const mSnap=await getDocs(collection(db,'matches'));const matches=mSnap.docs.map(d=>({id:d.id,...d.data()})).filter(m=>m.status==='confirmed');
  const pSnap=await getDocs(collection(db,'players'));const players=pSnap.docs.map(d=>({id:d.id,...d.data()})).filter(p=>p.status==='active');
  const now=Date.now();
  ...
  const topFor=ms=>players.reduce((best,p)=>{const c=ms.filter(m=>m.playerAId===p.id||m.playerBId===p.id).length;return c>best.count?{...p,count:c}:best;},{count:0});
  const dayTop=topFor(dayM),weekTop=topFor(weekM),monthTop=topFor(monthM),updates=[];
  if(dayTop.count>0){await grantAward(dayTop.id,dayTop.name,'Most Active Day','isMostActiveD','mostActiveDCount');updates.push('Day: '+dayTop.name);}
  if(weekTop.count>0){await grantAward(weekTop.id,weekTop.name,'Most Active Week','isMostActiveW','mostActiveWCount');updates.push('Week: '+weekTop.name);}
  if(monthTop.count>0){await grantAward(monthTop.id,monthTop.name,'Most Active Month','isMostActiveM','mostActiveMCount');updates.push('Month: '+monthTop.name);}
  $('awardResult').textContent='Most Active: '+updates.join(' | ');T('Most Active badges awarded','success');
}
...
async function getTopPlayerInWindow(windowMs){
  ...
  const players=pSnap.docs.map(d=>({id:d.id,...d.data()})).filter(p=>p.status==='active');
  const matches=mSnap.docs.map(d=>({id:d.id,...d.data()})).filter(m=>m.status==='confirmed');
  const now=Date.now(),filtered=windowMs===Infinity?matches:matches.filter(m=>now-(m.createdAt?.toDate?.()?.getTime()||0)<windowMs);
  const playerCatMap={};players.forEach(p=>playerCatMap[p.id]=p.category||'Main');
  let best=null,bestRating=-Infinity;
  for(const p of players){
    const pm=filtered.filter(m=>m.playerAId===p.id||m.playerBId===p.id);if(!pm.length)continue;
    let w=0,d=0,l=0,gf=0,ga=0,cs=0;
    pm.forEach(m=>{const side=m.playerAId===p.id?'A':'B';const res=getRC(m.scoreA,m.scoreB,side);const myG=side==='A'?m.scoreA:m.scoreB,opG=side==='A'?m.scoreB:m.scoreA;const oppCat=playerCatMap[side==='A'?m.playerBId:m.playerAId]||'Main';const mult=getRatingMultiplier(p.category||'Main',oppCat,res);if(res==='W')w+=mult;else if(res==='D')d++;else l+=mult;gf+=myG;ga+=opG;if(opG===0)cs++;});
    const r=Math.max(0,w*10+d*5+l*(-5)+(gf-ga)+cs*2);if(r>bestRating){bestRating=r;best={...p};}
  }
  return best;
}
async function grantAward(pid,name,awardLabel,field,countField){
  const ref=doc(db,'players',pid),snap=await getDoc(ref);if(!snap.exists())return;
  const current=snap.data()[countField]||0;
  const allSnap=await getDocs(collection(db,'players'));
  const clearOthers=allSnap.docs.filter(d=>d.id!==pid&&d.data()[field]).map(d=>updateDoc(doc(db,'players',d.id),{[field]:false}));
  await Promise.all([updateDoc(ref,{[field]:true,[countField]:current+1}),...clearOthers]);
}
async function postSystemNews(text){try{await addDoc(collection(db,'systemNews'),{text,createdAt:serverTimestamp(),type:'award'});}catch(e){console.error(e);}}
async function loadSystemNews(){
  try{
    const snap=await getDocs(collection(db,'systemNews'));
    const items=snap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0)).slice(0,10);
    if(!items.length)return;
    const existing=$('newsText').textContent;
    $('newsText').textContent=items.map(n=>n.text).join('     |     ')+(existing?' | '+existing:'');
    $('newsTicker').style.display='flex';
  }catch(e){console.error(e);}
}
async function confirmSeasonReset(){
  ...
  try{
    const [pSnap,mSnap]=await Promise.all([getDocs(collection(db,'players')),getDocs(collection(db,'matches'))]);
    await Promise.all([...pSnap.docs.map(d=>updateDoc(doc(db,'players',d.id),{wins:0,draws:0,losses:0,goalsFor:0,goalsAgainst:0,cleanSheets:0,form:[],division:9,cycleMP:0,cyclePts:0,eloRating:DEFAULT_ELO,highestDivision:9})),...mSnap.docs.map(d=>deleteDoc(doc(db,'matches',d.id)))]);
...
function switchAdminTab(tab,btn){
  ...
  $('adminTab-'+tab).style.display='block';loadAdminPanel(tab);
}

// ── Assign all to window so onclick="" works ──
